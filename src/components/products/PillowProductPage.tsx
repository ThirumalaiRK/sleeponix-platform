...
import { pillows, Pillow } from './pillowData';
import { useCart } from '../../context/CartContext';
import { motion } from 'framer-motion';
import { CheckCircle, ShoppingCart, ChevronRight, Leaf, Wind, ShieldCheck, Recycle } from 'lucide-react';
import PillowCard from '../PillowCard';
import ProductSlider from '../ProductSlider';

const PillowProductPage: React.FC = () => {
  const { pillowId } = useParams<{ pillowId: string }>();
  const [pillow, setPillow] = useState<Pillow | null>(null);
  const [activeTab, setActiveTab] = useState('overview');
  const { addToCart, setCartOpen } = useCart();
  const navigate = useNavigate();

  useEffect(() => {
    const currentPillow = pillows.find(p => p.id === pillowId);
    if (currentPillow) {
      setPillow(currentPillow);
    } else {
      navigate('/404');
    }
    window.scrollTo(0, 0);
  ...
  const handleAddToCart = () => {
    if (pillow) {
      addToCart({
        ...
        category: 'Pillow',
        alt: pillow.name,
        description: pillow.tagline || pillow.name,
        warranty: pillow.specifications['Warranty'] || '1 Year',
      }, 1);
      setCartOpen(true);
    }
  };
  ...
  if (!pillow) {
    return <div className="h-screen bg-[#FAF9F6]"></div>; // Loading state
  }

  const relatedProducts = pillows.filter(p => p.id !== pillow.id);
  ...
  const featureIcons = {
    "Natural Latex Foam": <Leaf className="w-6 h-6 text-[#C2A66D]" />,
    "Breathable & Hypoallergenic": <Wind className="w-6 h-6 text-[#C2A66D]" />,
    "Removable Cotton Cover": <ShieldCheck className="w-6 h-6 text-[#C2A66D]" />,
    "Orthopedic Support": <Recycle className="w-6 h-6 text-[#C2A66D]" />,
  };
    ...
    <div className="bg-[#FAF9F6] text-[#1C2635]">
      ...
      <section className="relative h-[560px] lg:h-[640px] flex items-center justify-center text-white">
        <div className="absolute inset-0 bg-black overflow-hidden">
          <img src="/images/pillow-hero-bg.jpg" alt="Comfortable sleep" className="w-full h-full object-cover opacity-50" />
        ...
        >
          <div className="text-left">
            ...
            >
              <span className="font-sans text-4xl font-bold text-white">₹{pillow.price.toLocaleString('en-IN')}</span>
          ...
          >
            {pillow.highlights.map((highlight, index) => (
      ...
      <section className="py-16 sm:py-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {activeTab === 'overview' && <p>{pillow.description}</p>}
            {activeTab === 'specifications' && (
              <ul className="list-disc pl-5 space-y-4">
                {Object.entries(pillow.specifications).map(([key, value]) => (
                  <li key={key}>
            ...
            {activeTab === 'care' &&
              <div className="space-y-6">
                <div>
                  ...
                  <p>To keep your pillow fresh, remove and machine wash the cover in cold water. Tumble dry on low. The latex core should not be washed, but can be spot-cleaned with a damp cloth if necessary. Air dry away from direct sunlight.</p>
                </div>
                <div>
                  <h3 className="font-serif text-2xl text-[#1C2635] mb-4">Warranty</h3>
                  <p>This pillow comes with a {pillow.specifications['Warranty'] || 'standard warranty'}. Please refer to our warranty page for full details.</p>
                </div>
              </div>
            }
          </motion.div>
      ...
      <section className="bg-white py-16 sm:py-24 overflow-hidden border-t border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <ProductSlider
            ...
            renderCard={(relatedPillow) => (
              <PillowCard
                {...relatedPillow}
                onAddToCart={() => {
                  addToCart({
                    id: relatedPillow.id,
                    name: relatedPillow.name,
                    price: relatedPillow.price,
                    image: relatedPillow.image,
                    size: relatedPillow.size,
                    thickness: 'Standard',
                    category: 'Pillow',
                    alt: relatedPillow.name,
                    description: relatedPillow.tagline || relatedPillow.name,
                    warranty: relatedPillow.specifications['Warranty'] || '1 Year',
                  }, 1);
                  setCartOpen(true);
                }}
                onCardClick={() => navigate(`/products/pillows/${relatedPillow.id}`)}
              />
            )}
          />
        </div>
