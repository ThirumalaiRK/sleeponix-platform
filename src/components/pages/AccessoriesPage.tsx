...
import { ShieldCheck, Wind, Droplets, ArrowLeftRight } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import AccessoryCard from "../AccessoryCard";
import { useCart } from "../../context/CartContext";
import { toast } from "react-hot-toast";
import { accessories, Accessory } from "../products/accessories";

const features = [
  { icon: Droplets, text: '100% Waterproof' },
  { icon: Wind, text: 'Breathable Fabric' },
  { icon: ShieldCheck, text: 'Protects Your Mattress' },
  { icon: ArrowLeftRight, text: 'Elastic and Wrinkle-Free' },
];
...
const AccessoriesPage: React.FC = () => {
  ...
  const handleAddToCart = (accessory: Accessory) => {
    addToCart(accessory as any, 1);
    toast.success(`✅ ${accessory.name} added to your cart!`);
  };

  const handleCardClick = (accessory: Accessory) => {
    navigate(`/products/accessories/${accessory.id}`);
  };
      ...
      >
        ...
        <div className="relative max-w-4xl mx-auto px-4">
          <h1 className="text-5xl md:text-6xl font-serif font-bold text-gray-800">
            Accessories — Enhance Your Sleep Experience.
          </h1>
          <p className="mt-6 text-lg md:text-xl text-gray-600 max-w-2xl mx-auto">
            Protect and perfect your bed with our premium accessories, designed for comfort and durability.
          </p>
        </div>
      </motion.section>
            ...
            {accessories.map((accessory, index) => (
              ...
              >
                <AccessoryCard
                  ...
                  onCardClick={() => handleCardClick(accessory)}
                />
              </motion.div>
            ))}
          </div>
